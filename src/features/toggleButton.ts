 ...
 */
interface ToggleButtonPosition {
  left: number;
  top: number;
}
 ...
 */
function initDrag(toggleBtn: HTMLButtonElement): void {
 ...
 * @param {HTMLButtonElement} toggleBtn
 */
function keepButtonInViewport(toggleBtn: HTMLButtonElement): void {
  const rect = toggleBtn.getBoundingClientRect();
  const nextPosition = clampPosition(
    rect.left,
    rect.top,
...
function restorePosition(toggleBtn: HTMLButtonElement): void {
  const savedPosition = storageGet<ToggleButtonPosition>(POSITION_STORAGE_KEY);
...
function savePosition(toggleBtn: HTMLButtonElement): void {
  const rect = toggleBtn.getBoundingClientRect();
