...
- **Source modules**:
  ...
  - [fingerprint/segments.ts](../src/features/navigation/fingerprint/segments.ts): Builds and merges tab-scoped derived estimates from unrendered text and observed viewport segments from real DOM Range geometry; the active search controller does not consume them yet.
